...
export async function ensureProfileOnSignIn(userId, email, name, client = redis) {
  ...
  if (existing) {
    // משתמש שבקשתו בוטלה (pending + requestActive=false): בכניסה מחדש הוא "מתחיל
    // בקשה חדשה" — מפעילים מחדש את הבקשה כדי שיחזור להיספר כ"טרם השלים".
    if (existing.approvalStatus === APPROVAL.PENDING && existing.requestActive === false) {
      const reactivated = { ...existing, requestActive: true, updatedAt: new Date().toISOString() };
      await client.set(PROFILE_KEY(userId), reactivated);
      return reactivated;
    }
    return existing;
  }
  const profile = {
    ...
    requestActive: true, // בקשה פעילה מרגע הכניסה
...
// onboardingCompletedAt=null. המשתמש אינו נחסם ואינו מאושר, וחוזר אוטומטית לטופס
// ההגשה בכניסה הבאה. מותרת מ-pending (ביטול בקשה) ומ-rejected (ביטול דחייה/הסרת
// חסימה); לא תלויה ב-referralSource (עובדת גם כשריק/חסר).
export async function cancelRequest(userId, actingAdminId, client = redis) {
  ...
  const updated = {
    ...
    referralSource: null,
    onboardingCompletedAt: null,
    requestActive: false, // אין יותר בקשה פעילה → מוסתר מהניהול עד כניסה מחדש
    updatedAt: new Date().toISOString(),
    lastChangedBy: actingAdminId || null,
  };
  await client.set(PROFILE_KEY(userId), updated);
...
export async function setReferral(userId, text, adminEmailsCsv, client = redis) {
  ...
  const updated = {
    ...
    requestActive: true, // מילוי הטופס = בקשה פעילה
    updatedAt: now,
  };
  await client.set(PROFILE_KEY(userId), updated);
  return { ok: true, profile: updated };
