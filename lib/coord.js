...
import { canCancelRequest } from './authz.js';
...
// ביטול בקשה — רק שולח הבקשה (requesterId) או Admin. הבדיקה על הרשומה הטרייה
// בתוך המוטטור (בעלות requesterId היא באותו collection).
export const mutatorCancelOwned = (id, access, userId) => (list) => {
  const target = list.find(c => c.id === id);
  if (!target) return { block: true, reason: 'not_found', httpStatus: 404 };
  if (!canCancelRequest(access, userId, target)) return { block: true, reason: 'forbidden', httpStatus: 403 };
  return list.map(c => c.id === id ? { ...c, requestStatus: 'cancelled', updatedAt: nowIso() } : c);
};
