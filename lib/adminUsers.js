...
// שיוך משתמש לדלי: Admin (נגזר) → 'admins' תמיד. pending מפוצל לפי השלמת הטופס:
// השלים → 'pending' (ממתין לאישור), טרם השלים → 'incomplete' (טרם השלים/בוטל).
// approved/rejected/disabled לפי הסטטוס; ערך לא-מוכר → 'incomplete'.
export function bucketOf(user) {
  if (user?.isAdmin) return 'admins';
  const s = user?.approvalStatus;
  if (s === 'pending') return user?.onboardingCompletedAt ? 'pending' : 'incomplete';
  return APPROVAL_STATES.includes(s) ? s : 'incomplete';
}

// מונה לכל דלי. כל משתמש נספר בדלי אחד בדיוק; 'all' = הכול.
