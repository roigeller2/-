...
// (התיקון לבאג ה-null-flicker). אותו מפתח ('coordination-requests') ואותו פורמט אחסון.
...
export async function PUT(request) {
  try {
    ...
    if (!Array.isArray(result) || result.length !== 3) {
      console.error('[api/coordination-requests] PUT: מבנה תשובה לא צפוי מ-eval:', result);
    ...
    if (successNum !== 0 && successNum !== 1) {
      console.error('[api/coordination-requests] PUT: successFlag לא צפוי מ-eval:', successFlag);
      return NextResponse.json({ ok: false, error: 'תשובה לא צפויה מהשרת (CAS flag)' }, { status: 500 });
    }

    console.log(
      `[api/coordination-requests] PUT expectedRev=${expectedRev} actualRev=${actualRev} newRev=${newRev} ` +
